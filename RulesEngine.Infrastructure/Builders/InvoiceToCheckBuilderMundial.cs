...
{
    public class InvoiceToCheckBuilderMundial : IInvoiceToCheckBuilder
    {
        ...
        public InvoiceToCheckBuilderMundial(IEnumerable<IExternalDataLoader> loaders,
        ...
        public async Task<IInvoiceToCheckContext> BuildAsync(string radNumber, string moduleName, string stage, string tenant)
        {
            ...
            var _loader = _loaders.FirstOrDefault(l => string.Equals(l.Tenant, tenant, StringComparison.OrdinalIgnoreCase))
            ...
            var invoice = new InvoiceToCheck(radNumber, _loader);
            invoice = await new LoadInputSourcesMundial(invoice, invoiceData, stage, inputSources, _invoiceRepository, _utilityService).Create();
