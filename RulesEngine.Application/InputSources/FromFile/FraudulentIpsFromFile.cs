...
{
    public class FraudulentIpsFromFile : IInputSource<FraudulentIps>
    {
        private readonly string _connectionString;
        private readonly SharedResource _resource;

        private const int nitIps = 1;
        private const int nameIps = 2;
        private const int notificationDate = 6;
        private const int result = 7;

        public FraudulentIpsFromFile(string connectionString, SharedResource resource)
        {
            _connectionString = connectionString;
            _resource = resource;
        }

        public async Task<List<FraudulentIps>> Create()
        {
                        ...
                        using (var st = await file.OpenReadAsync())
                        {
                            using(StreamReader streamReader = new StreamReader(st))
                            {
                                ...
                                var entities = lines.Select(line =>
                                {
                                    ...
                                    return new FraudulentIps
                                    {
                                        IpsNit = values[nitIps],
                                        IpsName = values[nameIps],
