...
{
    public class UsersAllowedClaimFromFile :IInputSource<AllowedUser>
    {
        private const int UserAccount = 0;
        private readonly SharedResource _resource;
        private readonly string _connectionString;
        public UsersAllowedClaimFromFile(string connectionString, SharedResource resource)
        {
            _resource = resource;
            _connectionString = connectionString;
        }

        public async Task<List<AllowedUser>> Create()
        {
                        ...
                        using (var st = await file.OpenReadAsync())
                        {
                            using (StreamReader streamReader = new StreamReader(st))
                            {
                                ...
                                var entities = lines.Select(line =>
                                {
                                    ...
                                    return new AllowedUser { UserAccount = values[UserAccount], };
                                }).ToList();
