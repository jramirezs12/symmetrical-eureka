...
{
    public class ServiceCodesFromFile : IInputSource<ServiceCodeFile>
    {
        private const int ServiceCodeColumn = 0;
        private readonly SharedResource _resource;
        private readonly string _connectionString;
        public ServiceCodesFromFile(string connectionString, SharedResource sharedResource)
        {
            _resource = sharedResource;
            _connectionString = connectionString;
        }
        public async Task<List<ServiceCodeFile>> Create()
        {
                        ...
                        using (var st = await file.OpenReadAsync())
                        {
                            using (StreamReader streamReader = new StreamReader(st))
                            {
                                ...
                                var entities = lines.Select(line =>
                                {
                                    ...
                                    return new ServiceCodeFile
                                    {
                                        ServiceCode = values[ServiceCodeColumn]
                                    };
                                }).ToList();
